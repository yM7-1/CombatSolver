...
- 下一步机制候选（由归因得出）：`ecaf220d` 的丢点是 **准入支配拒绝**（step2 `rejected_dominated`，前置双药投资的状态被更省的支配态拒绝），租约只加剪枝席位覆盖不到；可评估「有增量/投资进展的节点在准入支配比较中豁免」的小改动（单线程根归因 + broad-12 零退化门）。`9434499f` 的丢点是 `outer_prune_dropped`，需要保留池排名对增量节点的席位保护。
